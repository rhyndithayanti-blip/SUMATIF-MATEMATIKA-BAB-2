              ...
              {onResetDefaultQuestions && (
                <button
                  ...
                  onClick={() => {
                    if (
                      confirm(
                        `Kembalikan seluruh bank soal ke 20 butir standar (10 PG, 5 PGK, 5 PGK Kategori)?`
                      )
                    ) {
                      onResetDefaultQuestions();
                    }
                  ...
                  className="inline-flex items-center gap-1.5 px-3 py-2 text-xs font-semibold text-slate-700 bg-slate-100 hover:bg-slate-200 rounded-xl transition-colors border border-slate-300 cursor-pointer"
                  title="Reset seluruh soal ke 20 butir standar Matematika Kurikulum Merdeka"
                >
                  <RotateCcw className="w-3.5 h-3.5 text-slate-600" />
                  <span>Reset ke 20 Soal Standar</span>
                </button>
              )}
              <button
          ...
          <div className="space-y-3">
            {questions.map((q) => (
              ...
              >
                ...
                {/* Tampilan Diagram jika ada */}
                {q.diagram && (
                  <div className="my-2">
                    <FractionDiagram diagram={q.diagram} compact />
                  </div>
                )}
